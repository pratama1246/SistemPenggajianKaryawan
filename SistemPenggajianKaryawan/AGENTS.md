...
-- Tabel konfigurasi jam kerja (dikelola Admin)
...
Signature baru (breaking change):
```csharp
...
Per hari hadir:
  Telat > toleransi  → potong (menit telat / 60) x upah per jam
  Pulang cepat       → potong jam_kurang x upah per jam
  Lembur             → tambah jam_lembur x upah per jam x 1.5
